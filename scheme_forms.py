    ...
    while expressions is not nil:
        ...
        if is_scheme_true(test):
            ...
            if clause.rest is nil:
                return test  # return the eval'd predicate if there is no sub expression
            else:
                return eval_all(clause.rest, env)  # return the eval'd sub expression
            # if there is no true predicate or else, this returns None
            # END PROBLEM 13
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        list_names.append(binding.first)
        list_values.append(scheme_eval(binding.rest.first, env))
        bindings = bindings.rest

    for symbol in reversed(list_names):
        names = Link(symbol, names)
    for value in reversed(list_values):
        vals = Link(value, vals)
